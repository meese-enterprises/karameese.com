...
<html>
    <head>
        <title>AaronOS Messenger Standalone</title>
        <style>
            ::-webkit-scrollbar{
                width:16px;
    ...
    <body id="pagebody">
        <div id="winMSGh"></div>
        <div id="phpvars" style="display:none">{<?php
            if(!isset($_COOKIE['msgUser'])){
                $_COOKIE['msgUser'] = strval(intval(file_get_contents('users.txt')) + 1);
                while(file_exists('users/u_'.$_COOKIE['msgUser'].'.txt')){
                    $_COOKIE['msgUser'] = strval(intval($_COOKIE['msgUser']) + 1);
                }
                file_put_contents('users/u_'.$_COOKIE['msgUser'].'.txt', '');
                setcookie('msgUser', $_COOKIE['msgUser'], time() + 321408000);
                file_put_contents('users.txt', $_COOKIE['msgUser']);
                echo '"userName": "",';
            }else if(preg_match('/[^0-9]/', $_COOKIE['msgUser'])){
                $_COOKIE['msgUser'] = strval(intval(file_get_contents('users.txt')) + 1);
                while(file_exists('users/u_'.$_COOKIE['msgUser'].'.txt')){
                    $_COOKIE['msgUser'] = strval(intval($_COOKIE['msgUser']) + 1);
                }
                ...
                echo '"userName": "'.file_get_contents('users/u_'.$_COOKIE['msgUser'].'.txt').'",';
            }
            if(!isset($_COOKIE['msgPass'])){
                echo '"needPass": "true",';
            }
            echo '"userID": "'.$_COOKIE['msgUser'].'"';
            // any other variables the script needs access to
        ?>}</div>
    </body>
    <script defer>
        ...
        var phpVars = JSON.parse(getId('phpvars').innerHTML);
        var apps = {};
        apps.messaging = {
            ...
            vars: {
                ...
                discussionTopic: 'AaronOS Messenger',
                lastMsgRecieved: '-9',
                nameTemp: 'Anonymous',
                name: 'Anonymous',
                xhttpDelay: 0,
                    ...
                    if(this.messageTemp === this.lastMessage){
                        ...
                        if(this.messageTemp.length !== 0){
                            ...
                            */
                            ...
                            this.sendhttp.open('POST', 'saveMessage.php');
                                ...
                                if(apps.messaging.vars.sendhttp.readyState === 4){
                                    if(apps.messaging.vars.sendhttp.status === 200){
                                        if(apps.messaging.vars.sendhttp.responseText === 'Error - Password incorrect.'){
                                            alert('Could not send message. Your password is incorrect.\n\nIf you recently set a new password, try to refresh the page, enter your new password, and see if that fixes the issue. If the issue persists, please contact the developer.\n\nIf you want to start a new account, visit newAccount.php');
                                        }else if(apps.messaging.vars.sendhttp.responseText === 'Error - Password not provided.'){
                                            alert('Could not send message. You did not provide a password.\n\nTry refreshing the page, and be sure to enter your password when prompted. If the issue persists, please contact the developer.\n\nIf you want to start a new account, visit newAccount.php');
                ...
                nextMessage: function(text){
                    ...
                    if(text[0] === '{'){
                        ...
                        this.lastUserRecieved = this.lastResponseObject.i;
                        if(this.needsScroll){
                ...
                recieveMessage: function(){
                    if(apps.messaging.vars.firstLoad){
                        ...
                        // ask user for name if needed
                        if(phpVars.userName === ""){
                            while(this.tempNewName === "" || this.tempNewName.indexOf("{ADMIN}") === 0){
                                this.tempNewName = prompt("Your ID number is " + phpVars.userID + ".\n\nPlease enter a username.");
                                if(this.tempNewName === ""){
                                    alert("Your username can't be blank! Try again.");
                                }else if(this.tempNewName.indexOf("{ADMIN}") === 0){
                                    alert("Admins are marked manually by the developer. Please contact the developer to get admin markers.");
                                }
                            }
                            while(this.tempNewPass === "" || this.tempNewPass !== this.tempPassConf){
                                this.tempNewPass = prompt("Your ID number is " + phpVars.userID + ".\n\nPlease enter a password.");
                                this.tempPassConf = prompt("Please re-enter your password to confirm it.");
                                if(this.tempNewPass === ""){
                                    alert("You must not use a blank password! Try again.");
                                }else if(this.tempNewPass !== this.tempPassConf){
                                    alert("You didn't re-enter the same password twice! Try again.");
                                }
                            }
                            document.cookie = 'msgPass=' + this.tempNewPass + "; Max-Age:315576000";
                        }else if(phpVars.needPass === "true"){
                            this.tempNewPass = prompt("Your ID number is " + phpVars.userID + ".\n\nWelcome, " + phpVars.userName + ".\nPlease enter your password.");
                            document.cookie = 'msgPass=' + this.tempNewPass + "; Max-Age:315576000";
                        }
                        // send request to save username if needed
                        if(this.tempNewName !== ""){
                            var temphttp = new XMLHttpRequest();
                            temphttp.open('POST', 'setUsername.php');
                            var tempfd = new FormData();
                            tempfd.append('u', this.tempNewName);
                            tempfd.append('p', this.tempNewPass);
                            temphttp.send(tempfd);
                        }
                    ...
                    this.xhttp.open("GET", "loadMessage.php?l=" + this.lastMsgRecieved, true);
