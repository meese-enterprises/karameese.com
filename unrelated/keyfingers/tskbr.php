...
<html style="overflow:hidden">
		<head>
				...
				<script defer>
						...
						window.requestAnimationFrame(function(){
								...
								big = 0;
								makeBig = function(sizes, taskbar){
										if(sizes){
												big = 1;
												...
												if(taskbar){
														boxDiv.style.top = window.innerHeight - sizes[1] + 'px';
														document.getElementById('btns').style.display = 'block';
														document.getElementById('btns').style.filter = 'invert(1)';
												}else{
														boxDiv.style.top = '30px';
														document.getElementById('btns').style.filter = '';
												}
										}else if(!big){
												big = 1;
												document.body.style.background = '#000';
								...
								renderFrame = function(){
										...
										if(!big){
												...
												for(var i = 0; i < 1024; i++){
														...
														vctx.lineTo(Math.round(i * transformFact[0]) + 0.5, Math.round(0 * transformFact[1]));
														vctx.stroke();
														if(Math.round((i + 1) * transformFact[0]) - Math.round(i * transformFact[0]) > 1 && i !== 1023){
																...
																vctx.lineTo(Math.round(i * transformFact[0]) + 1.5, Math.round(0 * transformFact[1]));
																vctx.stroke();
														}
												}
										}
								...
								startSong = function(file){
										...
										audio.oncanplaythrough = function(){
												if(loadsReady){
														if (!big) makeBig([window.innerWidth, parseInt(document.getElementById('tskbrSize').value)], 1);}
		...
		<body>
				...
				<audio id="msAudio" src="" onended="if(resetAfter){window.location='https://aaron-os-mineandcraft12.c9.io/unrelated/keyfingers/tskbr.php?random=true';}"></audio>
				<div id="loadDiv">
				...
				<div id="btns" style="position:absolute;top:0;left:0;z-index:100">
						...
						<button onclick="window.location='tskbr.php'">New</button>
						...
						<button onclick="window.location='https://aaron-os-mineandcraft12.c9.io/unrelated/keyfingers/tskbr.php?random=true';">Randomize</button>
						| <input id="tskbrSize" placeholder="size" value="40" size="4" style="font-family:monospace"> <button onclick="makeBig([window.innerWidth, parseInt(document.getElementById('tskbrSize').value)], 1)">Shrink to Taskbar</button>
				</div>
				<?php
