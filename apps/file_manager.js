		...
		vars: {
			...
			update: function () {
				...
				if (this.currLoc === "/") {
					...
					let temphtml = "";
					for (const item in this.currentDirectoryFiles) {
						if (this.currentDirectoryFiles[item]) {
							// if item is a folder
							if (
								this.currentDirectoryFiles[item][
									this.currentDirectoryFiles[item].length - 1
								] === "/" &&
								this.currentDirectoryFiles[item][
									this.currentDirectoryFiles[item].length - 2
								] !== "\\"
							) {
								temphtml +=
									...
									(this.currLoc + this.currentDirectoryFiles[item]) +
									"\\');toTop(apps.properties)'])\">" +
									'<img src="files/small/folder.png"> ' +
								...
								temphtml +=
									"<div class=\"cursorPointer\" oncontextmenu=\"ctxMenu([[event.pageX, event.pageY, 'ctxMenu/file.png', 'ctxMenu/x.png'], ' Properties', 'apps.properties.main(\\'openFile\\', \\'" +
									(this.currLoc + this.currentDirectoryFiles[item])
										.split("\\")
										.join("\\\\") +
									"\\');toTop(apps.properties)'])\">" +
